        ...
        <div class="col-12 col-md-12 col-lg-12">
            ...
            <table class="table table-striped table-bordered table-sm">
                ...
                @foreach ($data as $no => $val)
                    <tr>
                        <td>{{ $no+1 }}</td>
                        <td>{{ $val->karyawan_id }}</td>
                        <td>{{ $val->tahun }}</td>
                        <td>{{ $val->bulan }}</td>
                        <td>{{ $val->jumlah_hari_kerja }}</td>
                        <td>
                            <a href="#" data-id="{{ $val->karyawan_id }}" class="badge badge-warning btn-edit">Edit</a>
                            @can('hapus_data', \App\Models\Divisi::class)
                                <a href="#" data-id="{{ $val->karyawan_id }}" class="badge badge-danger swal-confirm">
                                    <form action="{{ route('divisi.destroy', $val->karyawan_id) }}" id="deleteForm{{ $val->karyawan_id }}" method="POST">
                                        @csrf
                                        @method('delete')
                                    </form>
                                    Delete
